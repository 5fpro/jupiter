...
RSpec.describe SlackChannel, type: :model do
  ...
  context 'FactoryBot' do
    ...
    it { FactoryBot.create :slack_channel, :record_created }
    it { FactoryBot.create :slack_channel, :approach_hours_limit }
  ...
  it '#events and & #event?' do
    ...
    expect(described_class.find(slack_channel.id).event?(:record_created)).to eq true
  end
end
