...
describe Project::HoursLimitCheckContext, type: :context do
  ...
  context 'success' do
    ...
    let!(:record) { FactoryBot.create :record, project: project, minutes: 100 }

    it { expect { subject.perform }.to change { project.reload.approached_hours_limit }.to(true) }

    context 'slack notify' do
      before { FactoryBot.create :slack_channel, :approach_hours_limit, project: project }
  ...
  context 'not approach' do
    ...
    let!(:record) { FactoryBot.create :record, project: project, minutes: 30 }
